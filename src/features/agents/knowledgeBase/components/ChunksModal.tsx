...
export function ChunksModal({ open, file, onClose }: ChunksModalProps) {
  ...
  const [chunks, setChunks] =
    useState<Record<string, { title: string; content: string; charCount: number }[]>>(
      mockChunksData
    );
  ...
  const handleAdd = () => {
    ...
    setChunks((prev) => ({
      ...prev,
      [file.id]: [
        { title: newTitle.trim(), content: newContent.trim(), charCount: newContent.trim().length },
        ...(prev[file.id] || []),
      ],
    }));
    setAdding(false);
  ...
  const handleEdit = (idx: number) => {
    ...
    setEditIdx(null);
    toast.success('已更新该分块内容');
  };

  const handleDelete = (idx: number) => {
    setChunks((prev) => ({
      ...prev,
      [file.id]: (prev[file.id] || []).filter((_, i) => i !== idx),
    }));
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xs text-muted-foreground mt-0.5">
                  解析器: {file.parser} · 分块数量: {fileChunks.length} 块
                </p>
              </div>
            </div>
            ...
            fileChunks.map((chunk, idx) => {
              if (editIdx === idx) {
                return (
                  <div
                    key={idx}
                    className="p-4 bg-amber-50/40 dark:bg-amber-950/10 rounded-2xl border border-amber-200/50 space-y-3"
                  >
                    ...
                    />
                    <div className="flex gap-2 justify-end">
                      <Button variant="outline" size="xs" onClick={() => setEditIdx(null)}>
                        取消
                      </Button>
                      <Button
                        size="xs"
              ...
              return (
                <div
                  key={idx}
                  className="p-4 bg-muted/30 rounded-2xl border border-border hover:border-primary/20 transition-all group relative"
                >
